...
def train(
    ...
    if not base_mod:
        e2c.load_state_dict(torch.load("conv_mul/conv"+str(seed)+"/base_mod.pth", map_location=device))
    ...
    if not os.path.exists("conv_mul/conv" + str(seed)):
        os.mkdir("conv_mul/conv" + str(seed))
    if not base_mod:
        torch.save(e2c.state_dict(), "conv_mul/conv"+str(seed)+"/mod" + str(beta) + ".pth")
    else:
        torch.save(e2c.state_dict(), "conv_mul/conv"+str(seed)+"/base_mod.pth")
    ...
    if debug_vis:
        ...
        if not base_mod:
            plt.savefig("conv_mul/conv"+str(seed)+"/plot" + str(beta) + ".png")
            plt.show()
